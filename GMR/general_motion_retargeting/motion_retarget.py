...
from .params import ROBOT_XML_DICT, IK_CONFIG_DICT, INLINE_IK_CONFIG_DICT
...
class GeneralMotionRetargeting:
    ...
    def __init__(
        ...
        # Load the IK config
        inline_config = INLINE_IK_CONFIG_DICT.get(src_human, {}).get(tgt_robot)
        if inline_config is not None:
            ik_config = inline_config
            if verbose:
                print("Use inline IK config for robot: ", tgt_robot)
        else:
        ...
        if use_velocity_limit:
            ...
            for actuator_id in range(self.model.nu):
                ...
                if joint_name is not None:
                    velocity_limit_joint_names[joint_name] = 3 * np.pi
            VELOCITY_LIMITS = velocity_limit_joint_names
