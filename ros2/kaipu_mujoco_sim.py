...
def parse_lock_config(lock_str: str) -> dict:
    ...
    config = {
        ...
        'lock_roll': False, 'lock_pitch': False, 'lock_yaw': False
    }
    ...
    # 展开简写（注意顺序：先处理长的，避免 xyz 被 xy 替换）
    if "xyz" in lock_str:
...
class MujocoMocapNode(Node):
    ...
    def __init__(
        ...
        tf_prefix: str = "",   # e.g. "mujoco/" if you want namespaced frames

        lock_config: Optional[dict] = None,  # 新增：灵活的锁定配置
        joint_damping: float = 1.0,  # 新增：关节阻尼系数
        gravity: Optional[float] = None,  # 新增：重力加速度（None表示使用XML中的值）
    ):
        super().__init__("mujoco_mocap_half_fixed_base")

        # 声明所有参数（使其在ROS2参数系统中可见）
        ...
        # 如果有lock_config，声明lock_freejoint参数
        if lock_config is not None:
            # 重建lock_freejoint字符串用于显示
            lock_parts = []
            if lock_config.get('lock_x'): lock_parts.append('x')
            if lock_config.get('lock_y'): lock_parts.append('y')
            if lock_config.get('lock_z'): lock_parts.append('z')
            if lock_config.get('lock_roll'): lock_parts.append('roll')
            if lock_config.get('lock_pitch'): lock_parts.append('pitch')
            if lock_config.get('lock_yaw'): lock_parts.append('yaw')
        ...
        # 锁定配置：如果提供了 lock_config，使用它；否则根据 lock_base_rotation 决定
        if lock_config is not None:
            ...
            self.lock_config = {
                'lock_x': False, 'lock_y': False, 'lock_z': False,
                'lock_roll': True, 'lock_pitch': True, 'lock_yaw': True
            }
        else:
            # 不锁定任何自由度
            self.lock_config = {
        ...
        # MuJoCo data is NOT thread-safe across ROS timers + sim thread.
        self._mj_lock = threading.Lock()

        # Prefer stable integrator in code (no XML edit)
        try:
        ...
        # Increase ground friction (no XML edit)
        ...
        for jid in range(self.m.njnt):
            if int(self.m.jnt_type[jid]) == int(mujoco.mjtJoint.mjJNT_FREE):
                self.free_qadr = int(self.m.jnt_qposadr[jid])     # 7: [x,y,z,qw,qx,qy,qz]
                self.free_dofadr = int(self.m.jnt_dofadr[jid])    # 6: [vx,vy,vz,wx,wy,wz]
        ...
        if self.publish_tf:
            ...
            # Precompute body names & parent indices
        ...
        self.cmd_applied = True  # 标记命令是否已应用
    ...
    def _frame_name(self, name: str) -> str:
        # Optional prefix; also avoid empty frame id
        if not name:
            name = "unnamed"
        if self.tf_prefix:
    ...
    def publish_tf_tree(self):
        """
        Publish MuJoCo body tree as TF:
        - For each body (except world body 0), publish transform from its parent body frame to itself.
        - Use mjData.xpos/xquat (world) to compute relative transform.
        """
        ...
        # Read all body world poses under lock (thread-safe)
        with self._mj_lock:
            # xpos: (nbody,3), xquat: (nbody,4) in [w,x,y,z]
        ...
        # Publish transforms for bodies 1..nbody-1
        # Body 0 is the world body in MuJoCo (name often "world" or empty).
        for bid in range(1, self.m.nbody):
            ...
            # Relative rotation: q_rel = q_p^{-1} * q_c
            q_rel = _quat_mul_wxyz(_quat_conj_wxyz(p_quat), c_quat)
            q_rel = _quat_normalize_wxyz(q_rel)

            # Relative translation in parent frame: t_rel = R_p^{-1} * (c_pos - p_pos)
            # Convert parent quat to rotation matrix using MuJoCo helper
            # mujoco.mju_quat2Mat expects (9,) output, quat [w,x,y,z]
    ...
    def on_cmd(self, msg: JointState):
        ...
        with self.cmd_lock:
            ...
            self.cmd_applied = False  # 标记有新命令需要应用
    ...
    def apply_half_fixed_base(self):
        ...
        if self.lock_config['lock_roll'] or self.lock_config['lock_pitch'] or self.lock_config['lock_yaw']:
            ...
            if self.lock_config['lock_roll']:
                self.d.qvel[self.free_dofadr + 3] = 0.0  # wx (roll rate)
            if self.lock_config['lock_pitch']:
                self.d.qvel[self.free_dofadr + 4] = 0.0  # wy (pitch rate)
            if self.lock_config['lock_yaw']:
                self.d.qvel[self.free_dofadr + 5] = 0.0  # wz (yaw rate)
